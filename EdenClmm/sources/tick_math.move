...
module eden_clmm::tick_math {
   ...
   // 常量定义
    const TICK_BOUND: u64 = 443636; // tick边界值
    const MAX_SQRT_PRICE_X64: u128 = 79226673515401279992447579055; // 最大sqrt价格（64位定点数）
    const MIN_SQRT_PRICE_X64: u128 = 4295048016; // 最小sqrt价格（64位定点数）

   // 错误代码
    const EINVALID_TICK: u64 = 1; // 无效的tick
   ...
   // 返回值：对应的tick索引
    public fun get_tick_at_sqrt_price(sqrt_price: u128): i64::I64 {
        ...
        // 计算log2(x) * 2^32
        let log_2_x32 = i128::shl(
            i128::sub(i128::from((msb as u128)), i128::from(64)),
            32
        );
        ...
        let tick_low =
            i128::as_i64(
                i128::shr(
                    i128::sub(log_sqrt_10001, i128::from(184467440737095516u128)), 64
                )
            );
        let tick_high =
            i128::as_i64(
                i128::shr(
                    i128::add(log_sqrt_10001, i128::from(15793534762490258745u128)),
                    64
                )
            );
   ...
   // 测试函数 - 测试根据sqrt价格获取tick
    ...
    fun test_get_tick_at_sqrt_price_1() {
        assert!(
            i64::eq(get_tick_at_sqrt_price(6469134034u128), i64::neg_from(435444))
                == true,
            0
        );
        assert!(
            i64::eq(
                get_tick_at_sqrt_price(13561044167458152057771544136u128),
                i64::from(408332u64)
            ) == true,
            0
        );
