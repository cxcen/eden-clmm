...
module eden_clmm::clmm_math {
    ...
    // 整数向下转换溢出
    const EINVALID_FIFRT_PRICE_INPUT: u64 = 5;
    // 无效的sqrt价格输入
    ...
    // 根据代币A数量计算流动性
    // 参数：
    //     - fifrt_price_0: 起始sqrt价格
    //     - fifrt_price_1: 结束sqrt价格
    //     - amount_a: 代币A数量
    //     - round_up: 是否向上取整
    // 返回值：流动性值
    public fun get_liquidity_from_a(
        ...
        let fifrt_price_diff =
            if (fifrt_price_0 > fifrt_price_1) {
                fifrt_price_0 - fifrt_price_1
            } else {
                fifrt_price_1 - fifrt_price_0
            };
        // 计算分子：fifrt_price_0 * fifrt_price_1 * amount_a
        let numberator =
            (full_math_u128::full_mul_v2(fifrt_price_0, fifrt_price_1) >> 64)
                * (amount_a as u256);
        let div_res = math_u256::div_round(
            numberator, (fifrt_price_diff as u256), round_up
        );
        (div_res as u128)
    }
    ...
    public fun get_liquidity_from_b(
        ...
        // 计算：(amount_b << 64) / fifrt_price_diff
        let div_res =
            math_u256::div_round(
                ((amount_b as u256) << 64),
                (fifrt_price_diff as u256),
                round_up
            );
        (div_res as u128)
    }
    ...
    public fun get_delta_a(
        ...
        if (fifrt_price_diff == 0 || liquidity == 0) {
            return 0
        };
        // 计算：(liquidity * fifrt_price_diff << 64) / (fifrt_price_0 * fifrt_price_1)
        let (numberator, overflowing) =
            math_u256::checked_shlw(
                full_math_u128::full_mul_v2(liquidity, fifrt_price_diff)
            );
        if (overflowing) {
            abort EMULTIPLICATION_OVERFLOW
        };
        let denominator = full_math_u128::full_mul_v2(fifrt_price_0, fifrt_price_1);
    ...
    public fun get_delta_b(
        ...
        // 计算：(liquidity * fifrt_price_diff) >> 64
        let lo64_mask = 0x000000000000000000000000000000000000000000000000ffffffffffffffff;
        let product = full_math_u128::full_mul_v2(liquidity, fifrt_price_diff);
    ...
    // 参数：
    //     - fifrt_price: 当前sqrt价格
    //     - liquidity: 流动性
    //     - amount: 代币A数量
    //     - by_amount_input: 是否按输入数量计算
    // 返回值：下一个sqrt价格
    public fun get_next_fifrt_price_a_up(
        fifrt_price: u128,
        liquidity: u128,
        amount: u64,
        ...
        // 计算新的sqrt价格
        let (numberator, overflowing) = math_u256::checked_shlw(full_math_u128::full_mul_v2(fifrt_price, liquidity));
        if (overflowing) {
            abort EMULTIPLICATION_OVERFLOW
        };

        let liquidity_shl_64 = (liquidity as u256) << 64;
        let product = full_math_u128::full_mul_v2(fifrt_price, (amount as u128));
        let new_fifrt_price =
            if (by_amount_input) {
        ...
        if (new_fifrt_price > tick_math::max_fifrt_price()) {
            abort ETOKEN_AMOUNT_MAX_EXCEEDED
        } else if (new_fifrt_price < tick_math::min_fifrt_price()) {
    ...
    //     - by_amount_input: 是否按输入数量计算
    // 返回值：下一个sqrt价格
    public fun get_next_fifrt_price_b_down(
        fifrt_price: u128,
        liquidity: u128,
        amount: u64,
        by_amount_input: bool
    ): u128 {
        // 计算delta_fifrt_price：(amount << 64) / liquidity
        let delta_fifrt_price = math_u128::checked_div_round(((amount as u128) << 64), liquidity, !by_amount_input);
        let new_fifrt_price =
            if (by_amount_input) {
                fifrt_price + delta_fifrt_price
            } else {
                fifrt_price - delta_fifrt_price
            };
    ...
    //     - a_to_b: 是否从A到B的交换
    // 返回值：下一个sqrt价格
    public fun get_next_fifrt_price_from_input(
        fifrt_price: u128,
        liquidity: u128,
        amount: u64,
        a_to_b: bool
        ...
        if (a_to_b) {
            get_next_fifrt_price_a_up(fifrt_price, liquidity, amount, true)
        } else {
            get_next_fifrt_price_b_down(fifrt_price, liquidity, amount, true)
        }
    }
    ...
    //     - a_to_b: 是否从A到B的交换
    // 返回值：下一个sqrt价格
    public fun get_next_fifrt_price_from_output(
        fifrt_price: u128,
        liquidity: u128,
        amount: u64,
        a_to_b: bool
        ...
        if (a_to_b) {
            get_next_fifrt_price_b_down(fifrt_price, liquidity, amount, false)
        } else {
            get_next_fifrt_price_a_up(fifrt_price, liquidity, amount, false)
        }
    }

    // 根据输入计算上方delta值（版本2）
    // 参数：
    //     - current_fifrt_price: 当前sqrt价格
    //     - target_fifrt_price: 目标sqrt价格
    //     - liquidity: 流动性
    //     - a_to_b: 是否从A到B的交换
    // 返回值：delta值
    public fun get_delta_up_from_input_v2(
        ...
        let fifrt_price_diff =
            if (current_fifrt_price > target_fifrt_price) {
                current_fifrt_price - target_fifrt_price
            } else {
                target_fifrt_price - current_fifrt_price
            };
        if (fifrt_price_diff == 0 || liquidity == 0) {
            return 0
        };
        if (a_to_b) {
            // A到B交换：计算代币A的数量
            let (numberator, overflowing) =
                math_u256::checked_shlw(full_math_u128::full_mul_v2(liquidity, fifrt_price_diff));
            if (overflowing) {
                abort EMULTIPLICATION_OVERFLOW
            };
            let denominator = full_math_u128::full_mul_v2(current_fifrt_price, target_fifrt_price);
    ...
    public fun compute_swap_step(
        ...
        let next_fifrt_price = target_fifrt_price;
        let amount_in: u64 = 0;
        let amount_out: u64 = 0;
        let fee_amount: u64 = 0;
        if (liquidity == 0) {
            return (amount_in, amount_out, next_fifrt_price, fee_amount)
        };
        if (a2b) {
            assert!(current_fifrt_price >= target_fifrt_price, EINVALID_FIFRT_PRICE_INPUT)
        } else {
            assert!(current_fifrt_price < target_fifrt_price, EINVALID_FIFRT_PRICE_INPUT)
        };
        ...
        if (by_amount_in) {
            ...
            if (max_amount_in > (amount_remain as u256)) {
                ...
                next_fifrt_price = get_next_fifrt_price_from_input(current_fifrt_price, liquidity, amount_remain, a2b);
            } else {
                ...
                next_fifrt_price = target_fifrt_price;
            };
            amount_out =
                (
            ...
            if (max_amount_out > (amount as u256)) {
                amount_out = amount;
                next_fifrt_price = get_next_fifrt_price_from_output(
                    current_fifrt_price, liquidity, amount, a2b
                );
            } else {
        ...
        (amount_in, amount_out, next_fifrt_price, fee_amount)
    }
    ...
    public fun get_amount_by_liquidity(
        ...
        let lower_price = tick_math::get_fifrt_price_at_tick(tick_lower);
        let upper_price = tick_math::get_fifrt_price_at_tick(tick_upper);
    ...
    public fun get_liquidity_from_amount(
        ...
        let lower_price = tick_math::get_fifrt_price_at_tick(lower_index);
        let upper_price = tick_math::get_fifrt_price_at_tick(upper_index);
        let amount_a: u64 = 0;
        let amount_b: u64 = 0;
        ...
        if (is_fixed_a) {
            ...
            if (i64::lt(current_tick_index, lower_index)) {
                ...
                _liquidity = get_liquidity_from_a(
                    current_fifrt_price, upper_price, amount, false
                );
            ...
            if (i64::gte(current_tick_index, upper_index)) {
                ...
                _liquidity = get_liquidity_from_b(
                    lower_price, current_fifrt_price, amount, false
                );
    ...
    fun test_get_liquidity_from_a() {
        ...
        let fifrt_price_0 = tick_math::get_fifrt_price_at_tick(i64::from(0));
        let fifrt_price_1 = tick_math::get_fifrt_price_at_tick(i64::from(1000));
        let amount_a = 1000000;
        assert!(
            get_liquidity_from_a(fifrt_price_0, fifrt_price_1, amount_a, false) == 20505166,
            0
        );

        //11188795550323325955 30412779051191548722 1000000000 959569283
        let fifrt_price_0 = tick_math::get_fifrt_price_at_tick(i64::neg_from(10000));
        let fifrt_price_1 = tick_math::get_fifrt_price_at_tick(i64::from(10000));
        let amount_a = 1000000000;
        assert!(
            get_liquidity_from_a(fifrt_price_0, fifrt_price_1, amount_a, false) == 959569283,
            0
        );

        //18437523468038800957 18455969290605290427 300000000000 300014987250637
        let fifrt_price_0 = tick_math::get_fifrt_price_at_tick(i64::neg_from(10));
        let fifrt_price_1 = tick_math::get_fifrt_price_at_tick(i64::from(10));
        let amount_a = 300000000000;
        assert!(
            get_liquidity_from_a(fifrt_price_0, fifrt_price_1, amount_a, false)
                == 300014987250637,
            0
        );
    ...
    fun test_get_liquidity_from_b() {
        ...
        assert!(
            get_liquidity_from_b(fifrt_price_0, fifrt_price_1, amount_b, false) == 19505166,
            0
        );
        ...
        assert!(
            get_liquidity_from_b(fifrt_price_0, fifrt_price_1, amount_b, false) == 959569283,
            0
        );
        ...
        assert!(
            get_liquidity_from_b(fifrt_price_0, fifrt_price_1, amount_b, false)
                == 300014987250637,
            0
        );
    ...
    fun test_get_next_price_a_up() {
        let (fifrt_price, liquidity, amount) = (10u128 << 64, 200u128 << 64, 10000000u64);
        let r1 = get_next_fifrt_price_a_up(fifrt_price, liquidity, amount, true);
        let r2 = get_next_fifrt_price_a_up(fifrt_price, liquidity, amount, false);
    ...
    fun test_get_next_price_b_down() {
        let (fifrt_price, liquidity, amount, add) =
            (
        ...
        let r = get_next_fifrt_price_b_down(fifrt_price, liquidity, amount, add);
        assert!(62058032627749460283664515391u128 == r, 0);
    }

    #[test]
    fun test_compute_swap_step() {
        let (current_fifrt_price, target_fifrt_price, liquidity, amount, fee_rate) =
            (1u128 << 64, 2u128 << 64, 1000u128 << 32, 20000, 1000u64);
        let (amount_in, amount_out, next_fifrt_price, fee_amount) =
            compute_swap_step(
                current_fifrt_price,
                target_fifrt_price,
                liquidity,
        ...
        assert!(next_fifrt_price == 18446744159608897937, 0);
        ...
        assert!(next_fifrt_price == 18446744159522998190, 0);
